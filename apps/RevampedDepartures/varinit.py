...
version = None
try:
    from skin_engine.versioning import find_version_marker
    version, version_marker = find_version_marker('.')
    if version_marker:
        with open(version_marker) as f:
            version_file = json.loads(f.read())
except Exception as exc:
    print('Version marker warning:', exc)
...
for all in settingstxt:
    try:settings[all]
    except: settings[all] = settingstxt[all]
