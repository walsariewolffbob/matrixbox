...
from skin_engine.versioning import skin_version, app_root
...
def discover_skins():
    for name in _skin_package_names():
        ...
        try:
            ...
            detected_version = skin_version(base)
            if detected_version is None:
                _REJECTED[name] = ('skin version marker not found',)
                print('SKIN rejected:', name, 'skin version marker not found')
                continue

            runtime_manifest = dict(getattr(module, 'manifest') or {})
            runtime_manifest['skin_version'] = detected_version
...
def fallback_renderer_mode():
    manifest = _MANIFESTS.get('sl_list')
    if manifest is not None:
        return int(manifest.get('legacy_listmode', 1))
    return default_renderer_mode()
