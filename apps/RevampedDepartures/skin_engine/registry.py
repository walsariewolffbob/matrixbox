...
from skin_engine.versioning import read_skin_metadata, app_root
...
# Compatibility aliases for skin IDs renamed without changing their legacy mode.
...
def discover_skins():
    for name in _skin_package_names():
        ...
        try:
            base = app_root().rstrip('/\\') + '/skins/' + name
...
def resync_skins(reload_modules=True):
    ...
    The desktop simulator can restart an app without restarting the Python
    interpreter.  Without an explicit resync that leaves removed skins in the
    registry and keeps old renderer class objects alive after files change.
    """
    if reload_modules:
        try:
            import sys
