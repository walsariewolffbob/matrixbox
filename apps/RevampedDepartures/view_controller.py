...
class ViewController:
    ...
    def _apply_mode_defaults(self, legacy_mode, functions):
        ...
        if combined:
            list_manifest = renderer_manifest('list')
            if list_manifest is not None:
                mode = int(list_manifest['legacy_listmode'])
    ...
    def _show_error_splash(self, functions, renderer_id):
        try:
            ...
            functions.sysprint('Loading List', 11, _refresh=True)
            time.sleep(1.5)
        except Exception as exc:
    ...
    def _recover_from_fault(self, functions, phase, renderer_id, exc,
        ...
        try:
            ...
            log_skin_fault('fallback', 'list', fallback_exc)
            self._exit_active()
            self.state = self.STATE_IDLE
            return False
        finally:
