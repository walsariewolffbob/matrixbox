...
try:
    from skin_engine.registry import renderer_view_options, renderer_ui_profiles
except Exception:
    renderer_view_options = None
    renderer_ui_profiles = None
...
<div class="card">
{LISTMODE_CHK}
<div id="view-size-warning" style="display:none;margin:8px 0 0;color:#d99;font-size:.78rem"></div>
{SCROLL_SECTION}
{DEVIATIONS_SECTION}
{CUSTOM_SCROLL_HTML}
{CLOCK_INLINE_HTML}
...
function updateScrollPositionOptions(c){
var sel=document.getElementById('custom_scroll_position');if(!sel)return;
var opts=c.scroll_position_options||[[0,'After departures'],[1,'Before departures'],[2,'After first departure']];
var current=String(sel.value),found=false;sel.innerHTML='';
opts.forEach(function(item){var o=document.createElement('option');o.value=String(item[0]);o.textContent=String(item[1]);if(o.value===current){o.selected=true;found=true;}sel.appendChild(o);});
if(!found&&sel.options.length)sel.selectedIndex=0;
}
function updateScrollControls(){
var lm=document.getElementById('listmode'),mode=lm?String(lm.value):'',p=VIEW_PROFILES[mode]||{},c=p.controls||{};
...
var supported=!!c.scroll_text,selector=!!c.scroll_selector,position=!!c.scroll_position;
var positionModes=c.scroll_position_modes||['custom'];
var positionEnabled=(isCustom&&positionModes.indexOf('custom')>=0)||(isDisruptions&&positionModes.indexOf('disruptions')>=0);
var delayEnabled=isDisruptions||(!!c.message_interval_custom&&isCustom);
...
if(seg)seg.parentNode.style.display=(supported&&selector)?'':'none';
if(sr)sr.style.display='none';
if(tr)tr.style.opacity=(!c.custom_text_requires_custom_mode||isCustom)?'1':'.35';
if(dr){dr.style.display=c.message_interval?'':'none';dr.style.opacity=delayEnabled?'1':'.35';}
if(pr){pr.style.display=position?'':'none';pr.style.opacity=positionEnabled?'1':'.35';}
if(ti)ti.disabled=!!c.custom_text_requires_custom_mode&&!isCustom;
if(di)di.disabled=!!c.message_interval&&!delayEnabled;
if(pi)pi.disabled=position&&!positionEnabled;
}
function setDlrScrollMode(v,el){
fetch('/?dlr_scroll_content='+encodeURIComponent(v));
var p=el.parentNode;if(p)p.querySelectorAll('button').forEach(function(b){b.classList.remove('on');});
el.classList.add('on');updateScrollControls();
}
...
var VIEW_PROFILES={VIEW_PROFILES_JSON};
var DISPLAY_SIZE='{DISPLAY_SIZE}';
function applyViewProfile(mode){
var p=VIEW_PROFILES[String(mode)]||{},c=p.controls||{},d=p.defaults||{};
var setDisplay=function(id,on){var e=document.getElementById(id);if(e)e.style.display=on?'':'none';};
setDisplay('custom-scroll-section',!!c.scroll_text);
setDisplay('custom-scroll-position-row',!!c.scroll_position);
setDisplay('custom-scroll-position-separator',!!c.scroll_position);
setDisplay('dlr-scroll-delay-row',!!c.message_interval);
setDisplay('classic-scroll-speed-section',!!c.scroll_speed);
setDisplay('clock-list-section',!!c.clock);
setDisplay('clock-list-extra',!!c.clock_extra);
setDisplay('clock-list-color-separator',!!c.clock_extra);
setDisplay('view-colour-section',!!c.line_minute_colour);
var wz=document.getElementById('view-size-warning'),sz=DISPLAY_SIZE,opt=p.optimized_display_sizes||[];if(wz){var ok=!opt.length||opt.indexOf(sz)>=0;wz.style.display=ok?'none':'';wz.textContent=ok?'':('Warning: '+p.name+' is not optimized for '+sz+'. Optimized for: '+opt.join(', '));}
var md=document.getElementById('maxdest');if(md&&p.departure_count){md.value=String(p.departure_count);md.disabled=false;}
var di=document.getElementById('dlr_scroll_delay');if(di&&d.dlr_scroll_delay!==undefined)di.value=String(d.dlr_scroll_delay);
var lc=document.getElementById('LISTCOLOR');if(lc&&d.listcolor!==undefined)lc.checked=!!Number(d.listcolor);
var lt=document.getElementById('LISTCOLOR_TIME');if(lt&&d.listcolor_time!==undefined)lt.checked=!!Number(d.listcolor_time);
var ct=document.getElementById('clocktime');if(ct&&d.clocktime!==undefined)ct.value=String(d.clocktime);
var nh=document.getElementById('night-bus-highlight');if(nh&&d.night_bus_highlight!==undefined){nh.querySelectorAll('button').forEach(function(b){b.classList.remove('on');});var nb=nh.querySelector('button[data-v="'+String(d.night_bus_highlight)+'"]');if(nb)nb.classList.add('on');}
var lds=document.getElementById('list-line-display');if(lds&&d.list_line_display!==undefined){lds.querySelectorAll('button').forEach(function(b){b.classList.remove('on');});var lb=lds.querySelector('button[data-v="'+String(d.list_line_display)+'"]');if(lb)lb.classList.add('on');}
updateScrollPositionOptions(c);
...
if(el.id==='listmode'){applyViewProfile(String(ev.target.value));}
});});
updateScrollControls();
...
def html():
    ...
    if if_long > 128:
        try:
            _multi_cols = max(2, min(3, int(s.get("multi_station_columns", 3))))
        except:
            _multi_cols = 3
        station_selection_html += ''.join([
            '<div class="toggle-row" id="multi-station-columns-row" style="', ('' if _combined_station_mode else 'display:none;'), '">',
            '<label for="multi_station_columns" class="toggle-label">Columns</label>',
            '<select id="multi_station_columns" class="form-control" style="width:130px;display:inline" data-p="multi_station_columns" data-e="change">',
            _opt(2, _multi_cols, '2 stations'), _opt(3, _multi_cols, '3 stations'),
            '</select></div>'
        ])
    ...
    if if_long == 128:
        _scroll_profile = {}
        try:
            if renderer_ui_profiles is not None:
                _scroll_profile = renderer_ui_profiles().get(str(int(s.get('listmode', 0))), {})
        except Exception:
            _scroll_profile = {}
        _scroll_show = "" if bool((_scroll_profile.get('controls', {}) or {}).get('scroll_speed', False)) else "display:none;"
        scroll_html = ''.join([
    ...
    if if_long > 64:
        _view_options = ()
        _view_registry_ok = False
        try:
            if renderer_view_options is not None:
                _view_options = renderer_view_options()
                _view_registry_ok = True
        except Exception:
            _view_options = ()

        if _view_registry_ok:
            _display_size = str(int(varinit.display.width)) + "x" + str(int(varinit.display.height))
            _compatible_views = []
            for _view in _view_options:
                _manifest = _view.get('manifest', {}) or {}
                _supported = tuple(_manifest.get('supported_display_sizes') or ())
                if not _supported or _display_size in _supported:
                    _compatible_views.append(_view)
            _view_options = tuple(_compatible_views)
        else:
            _view_options = (
                {'mode': 0, 'name': 'SL Classic'},
                {'mode': 1, 'name': 'List'},
                {'mode': 2, 'name': 'TfL DLR'},
            )

        if _view_options:
            _view_option_html = ''.join([
                _opt(_view['mode'], s['listmode'], _view['name']) for _view in _view_options
            ])
            listmode_html = ''.join([
    ...
    try:
        if renderer_ui_profiles is not None:
            _view_profiles = renderer_ui_profiles()
    except Exception:
        _view_profiles = {}
    _active_profile = _view_profiles.get(str(_view_mode_now), {})
    _active_controls = _active_profile.get('controls', {}) or {}
    _active_defaults = _active_profile.get('defaults', {}) or {}

    
    
    _cs_show = "" if bool(_active_controls.get('scroll_text', False)) else "display:none;"
    _cs_pos_show = "" if bool(_active_controls.get('scroll_position', False)) else "display:none;"
    _cs_position_options = _active_controls.get('scroll_position_options') or (
        (0, 'After departures'),
        (1, 'Before departures'),
        (2, 'After first departure'),
    )
    _cs_position_option_html = ''.join([
        _opt(value, s.get('custom_scroll_position', 0), label)
        for value, label in _cs_position_options
    ])
    ...
    _scroll_custom_disabled = "" if (not bool(_active_controls.get('custom_text_requires_custom_mode', False)) or _scroll_content == "custom") else " disabled"
    _scroll_custom_opacity = "1" if (not bool(_active_controls.get('custom_text_requires_custom_mode', False)) or _scroll_content == "custom") else ".35"
    _scroll_position_modes = _active_controls.get('scroll_position_modes') or ('custom',)
    _scroll_position_enabled = _scroll_content in _scroll_position_modes
    _scroll_position_disabled = "" if _scroll_position_enabled else " disabled"
    _scroll_position_opacity = "1" if _scroll_position_enabled else ".35"
    _scroll_delay_enabled = (_scroll_content == "disruptions" or
                             (bool(_active_controls.get('message_interval_custom', False)) and _scroll_content == "custom"))
    ...
    custom_scroll_html = ''.join([
        ...
        '<div id="dlr-scroll-mode-row" style="', ("" if bool(_active_controls.get('scroll_text', False)) else "display:none;"), 'margin-bottom:10px">',
        ...
        '<div id="custom-scroll-text-row" style="opacity:', (_scroll_custom_opacity if bool(_active_controls.get('scroll_text', False)) else "1"), '">',
        ...
        (_scroll_custom_disabled if bool(_active_controls.get('scroll_text', False)) else ""), '></div>',

        '<div style="border-top:1px solid var(--border);margin:10px 0 0"></div>',
        '<div id="dlr-scroll-delay-row" class="toggle-row" style="border-bottom:none;', ("" if bool(_active_controls.get('message_interval', False)) else "display:none;"), ';opacity:', _scroll_delay_opacity, '">',
        ...
        '<div id="custom-scroll-position-row" class="toggle-row" style="border-bottom:none;', _cs_pos_show, ';opacity:', (_scroll_position_opacity if bool(_active_controls.get('scroll_position', False)) else "1"), '"><label for="custom_scroll_position" class="toggle-label">Position</label>',
        ...
        (_scroll_position_disabled if bool(_active_controls.get('scroll_position', False)) else ""), '>',
        _cs_position_option_html,
        '</select></div>',
    ...
    _fs = "mini" if _combined_station_mode or s["mini"] else ("large" if s.get("large_list", 0) else "small")
    _font_dis = ' disabled' if _combined_station_mode else ''
    font_size_html = (
        '<span class="seg" style="' + ('opacity:.35;' if _combined_station_mode else '') + '">'
        '<button type="button" onclick="setFont(\'mini\',this)"' + _font_dis + ' class="' + ("on" if _fs == "mini" else "") + '">mini</button>'
        '<button type="button" onclick="setFont(\'small\',this)"' + _font_dis + ' class="' + ("on" if _fs == "small" else "") + '">small</button>'
        '<button type="button" onclick="setFont(\'large\',this)"' + _font_dis + ' class="' + ("on" if _fs == "large" else "") + '">large</button>'
        '</span>'
    )
    xs_line_id_chk = _chk("XS_LINE_ID", s.get("xs_line_id", 0), "/?xs_line_id=switch", "Show line ID") if if_long <= 64 else ""
    _clock_list_extra_show = "" if bool(_active_controls.get("clock_extra", False)) else "display:none;"
    clock_row_html = (
        ...
        + '</select></td></tr>'
        + '</table></div>'
        + '<div id="clock-list-color-separator" style="border-top:1px solid var(--border);margin:10px 0;' + ("" if bool(_active_controls.get('clock_extra', False)) else "display:none;") + '"></div>'
    ...
    _clock_show = "" if bool(_active_controls.get('clock', False)) else "display:none;"
    ...
    dest_scroll_html = _chk("DEST_SCROLL", 0 if _combined_station_mode else s.get("dest_scroll", 0), "/?dest_scroll=switch", "Scroll long destination names")
    _dsc = " checked" if int(s.get("dest_scroll", 0)) and not _combined_station_mode else ""
    _ds_dis = " disabled" if _combined_station_mode else ""
    dest_scroll_inline = '<label class="switch" style="' + ('opacity:.35' if _combined_station_mode else '') + '"><input type="checkbox" id="DEST_SCROLL" data-u="/?dest_scroll=switch"' + _dsc + _ds_dis + '><span class="slider"></span></label>'
    ...
    _listcolor_value = s["listcolor"]
    _listcolor_time_value = s.get("listcolor_time", 0)
    ...
    _line_display_default = int(_active_controls.get("line_display_default", _active_defaults.get("list_line_display", 1)))
    _line_display = int(s.get("list_line_display", _line_display_default))
    line_display_html = ''.join([
        '<div style="margin-bottom:10px"><label class="control-label" style="margin-bottom:6px">Line display</label>',
        '<span class="seg" id="list-line-display" style="width:100%">',
        '<button type="button" data-v="0" onclick="setListLineDisplay(0,this)" class="', ("on" if _line_display == 0 else ""), '">Departure order</button>',
        '<button type="button" data-v="1" onclick="setListLineDisplay(1,this)" class="', ("on" if _line_display == 1 else ""), '">Line number</button>',
        '</span></div>',
        '<div style="border-top:1px solid var(--border);margin:10px 0"></div>',
    ])
    ...
    _view_colour_show = "" if bool(_active_controls.get('line_minute_colour', False)) else "display:none;"
    ...
    _v = {
        ...
        "VIEW_PROFILES_JSON": json.dumps(_view_profiles, separators=(',', ':')),
        "DISPLAY_SIZE": str(int(varinit.display.width)) + "x" + str(int(varinit.display.height)),
