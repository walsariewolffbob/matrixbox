...
def huvudsidan(request):
    ...
    if "set_timer" in request.params:
        ...
        functions.request_view_rebuild()
        return (200, {}, "")
        ...
        functions.request_view_rebuild()
        return (200, {}, "")
    elif "multi_station_columns" in request.params:
        if varinit.if_long <= 128:
            varinit.settings["multi_station_columns"] = 2
        else:
            try:
                varinit.settings["multi_station_columns"] = max(2, min(3, int(request.params["multi_station_columns"])))
            except:
                return (400, {}, "")
        functions.request_view_rebuild()
        return (200, {}, "")
        ...
        if _station_mode == 1:
            ...
            _list_view = functions.skin_mode("list")
            if _list_view is not None:
                varinit.settings["listmode"] = _list_view
            varinit.settings["maxdest"] = 5
            ...
            if varinit.if_long <= 128:
                ...
                try:
                    varinit.settings["multi_station_columns"] = max(2, min(3, int(varinit.settings.get("multi_station_columns", 3))))
                except:
                    varinit.settings["multi_station_columns"] = 3
            if _old_view != _list_view:
        ...
        varinit.shared["classic_scroll_last_step"] = 0
        varinit.shared["classic_scroll_setting"] = int(varinit.settings["scroll"])
        return (200, {}, "")
        ...
        try:
            # The rendered View selector is already filtered to compatible
            # registered skins. Do not re-impose legacy physical-size gates
            # here: they prevented valid 128x64 (2X) skins from ever starting.
            functions.request_view_change(int(request.params["listmode"]))
        except Exception as e:
            print("VIEW selection error:", e)
        return (200, {}, "")
        ...
        if mode == "custom":
            ...
            varinit.settings["custom_scroll_show"] = 0
            varinit.settings["show_msgs"] = 0
        if functions.active_skin_capability("custom_scroll"):
            if functions.active_skin_is_classic_ticker():
        ...
        if int(varinit.settings.get("show_msgs", 0)):
            varinit.shared["disruption_timer"] = time.monotonic()
        if functions.active_skin_capability("message_interval"):
            functions.request_view_rebuild()
        return (200, {}, "")
        ...
        try:
            ...
            if functions.active_skin_capability("custom_scroll_position"):
                if functions.active_skin_is_classic_ticker():
                    varinit.shared["force_view_rebuild"] = 1
                else:
                    functions.request_view_rebuild()
        ...
        functions.request_view_rebuild(reset_timing=False)
        return (200, {}, "")
        ...
        if not varinit.settings["mins"]:
            varinit.settings["mins"] = str(dicts.settingstxt.get("mins", " min"))
        functions.request_view_rebuild()
        return (200, {}, "")
