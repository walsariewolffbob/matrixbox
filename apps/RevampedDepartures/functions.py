...
def handle_button_event(event):
    ...
    if event == 1:
        if int(varinit.settings.get("button_mode", 0)):
            ...
            try:
                from skin_engine.registry import renderer_view_options
                if _combined:
                    _next_mode = skin_mode("list")
                    if _next_mode is None:
                        return
                else:
                    _modes = [int(item["mode"]) for item in renderer_view_options()]
                    if not _modes:
                        return
                    _current = int(varinit.settings.get("listmode", _modes[0]))
                    try:
                        _index = _modes.index(_current)
                    except ValueError:
                        _index = -1
                    _next_mode = _modes[(_index + 1) % len(_modes)]

                request_view_change(_next_mode)
            except Exception as e:
                print("VIEW button cycle error:", e)
...
def view_switch_loading(_delay=2.0):
    # Skin renderers may temporarily reuse palette slots 3-5 for dynamic
    # colours. Restore the standard UI/logo palette before drawing the
    # changeover splash so the multicolour MatrixBOX M is deterministic.
    try:
        _ui_palette = {
            3: 0x000765,
            4: (100, 0, 0),
        ...
        for _index, _colour in _ui_palette.items():
            varinit.palette[_index] = _colour
            try:
                varinit.overlay_palette[_index] = _colour
            except Exception:
                pass
    except Exception as e:
        print("Splash palette restore warning:", e)
...
def active_skin_manifest():
    try:
        from skin_engine.registry import renderer_manifest_for_mode
        return renderer_manifest_for_mode(varinit.settings.get("listmode", 0)) or {}
    except Exception:
        return {}

def active_skin_capability(name, default=False):
    manifest = active_skin_manifest()
    return bool((manifest.get("capabilities", {}) or {}).get(name, default))

def active_skin_data_format():
    return str(active_skin_manifest().get("data_format", "rows"))

def active_skin_departure_count():
    manifest = active_skin_manifest()
    try:
        limit = int(manifest.get("departure_count", varinit.settings.get("maxdest", 1)))
    except Exception:
        limit = max(1, int(varinit.settings.get("maxdest", 1)))

    # Skins may expose a larger/smaller row capacity for a specific logical
    # display size without changing their normal-size behaviour.
    try:
        size = str(int(varinit.if_long)) + "x" + str(int(varinit.if_tall))
        by_size = manifest.get("departure_count_by_size", {}) or {}
        if size in by_size:
            limit = int(by_size[size])
    except Exception:
        pass

    combined = int(varinit.settings.get(
        "station_selection_mode",
        1 if int(varinit.settings.get("multiple", 0)) else 0
    )) == 1
    if combined:
        try:
            limit = int((manifest.get("combined_defaults", {}) or {}).get("maxdest", limit))
        except Exception:
            pass
    return max(1, limit)

def active_skin_is_classic_ticker():
    return active_skin_data_format() == "classic_ticker"

def skin_mode(renderer_id, fallback=None):
    try:
        from skin_engine.registry import renderer_manifest
        manifest = renderer_manifest(renderer_id) or {}
        return int(manifest.get("legacy_listmode"))
    except Exception:
        if fallback is None:
            return None
        return int(fallback)
...
def request_view_change(mode):
    """Request a registered View through the standard changeover path."""
    from skin_engine.registry import get_renderer_id_for_mode
    ...
    if combined:
        requested = skin_mode("list")
        if requested is None:
            return False

    if get_renderer_id_for_mode(requested) is None:
        return False

    current = int(varinit.settings.get("listmode", requested))
    if requested == current:
        return False

    varinit.settings["listmode"] = requested
    request_view_rebuild(splash=True, reset_timing=True)
    return True
...
def fetch_data(host, port=80, args="", headers = "", filetype="text"):
    if varinit.if_long > 128 and active_skin_is_classic_ticker(): version_delay(slowdown=1)
    ...
    print("--------------------------------------------------------- ")
    if varinit.if_long > 128 and active_skin_is_classic_ticker(): version_delay(slowdown=2)
...
def traffic_parser(data, traffic_type, num="1"):
    ...
    dataout = []
    _maxdest = min(
        max(1, int(varinit.settings.get("maxdest", 1))),
        active_skin_departure_count(),
    )
    ...
    for all in data["departures"]:
        ...
        # currenttime normally comes from the HTTP Date header. On a fast
        # startup/reload that header can occasionally fail to initialize it
        # before the first departures payload is parsed, leaving the original
        # empty-string value and causing ``float - str`` here. The API payload
        # itself carries an authoritative timestamp, so use that as the
        # fallback clock for this response.
        try:
            _now_epoch = float(varinit.currenttime)
        except (TypeError, ValueError):
            try:
                _now_epoch = float(_converttime(str(data.get("timestamp", ""))))
                varinit.currenttime = _now_epoch
            except Exception:
                try:
                    _now_epoch = float(time.mktime(time.localtime()))
                    varinit.currenttime = _now_epoch
                except Exception:
                    _now_epoch = 0.0
        difference = _converttime(all["expected"]) - _now_epoch
        ...
        if int(minsleft[:2]) > offset:
            ...
            if not int(all["direction_code"]) or not int(direction) or int(direction) == int(all["direction_code"]):
                ...
                if active_skin_capability("line_metadata", False):
                    dep.append({
                        "transport_mode": str(line.get("transport_mode", "")),
                        "group_of_lines": line.get("group_of_lines", ""),
                        "designation": str(line.get("designation", line.get("id", ""))),
                    })
...
def get_departure(num = "1", dataout = [["1", "^ Data error","","",""]]):
    ...
    if time.monotonic() > varinit.show_station_timer + varinit.show_station_interval and not varinit.shared["nightcount"]:
        if "demo" in os.listdir() or int(settings["show_my_station"]):
            ...
            if active_skin_is_classic_ticker(): renderstring(_out, 1,0,1, _cls=top)
...
def reformat_data(trainlist):
    ...
    elif not active_skin_is_classic_ticker(): return trainlist
...
def renderstring(_string, screen_partition = 0, min = 0, slow = 0, invertcolor = 0, shading=False, smallfont=False, sys_msg=False, shade=False, large=False, _cls=False, _refresh=False, ontop=False, block=False, logo=False, mini=False, start_x=0, clip_x=None, target_bmp=None, target_offs=None):
    ...
    if not ontop and active_skin_is_classic_ticker() and sys_msg:
        varinit.currentfont = 1
        varinit.tg3.y = 0
        
    if ontop: 
        screen_partition = 1
        if not active_skin_is_classic_ticker(): offs = 0
...
def custom_scroll_available():
    ...
    if not active_skin_capability("custom_scroll", False):
        return False
...
def savesettings(_settings=varinit.settings, saved=dicts.language[varinit.settings["language"]]["display"]["saving"]):
    ...
    if active_skin_capability("save_feedback_top_row"):
