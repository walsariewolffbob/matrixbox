...
var lm=document.getElementById('listmode'),mode=lm?String(lm.value):'',p=VIEW_PROFILES[mode]||{},c=p.capabilities||{},u=p.ui||{};
...
var supported=!!c.custom_scroll,selector=!!u.scroll_selector,position=!!c.custom_scroll_position;
var delayEnabled=isDisruptions||(!!u.message_interval_custom&&isCustom);
...
if(tr)tr.style.opacity=(!u.custom_text_requires_custom_mode||isCustom)?'1':'.35';
...
if(ti)ti.disabled=!!u.custom_text_requires_custom_mode&&!isCustom;
...
var p=VIEW_PROFILES[String(mode)]||{},c=p.capabilities||{},d=p.defaults||{},u=p.ui||{};
var setDisplay=function(id,on){var e=document.getElementById(id);if(e)e.style.display=on?'':'none';};
setDisplay('custom-scroll-section',!!c.custom_scroll);
setDisplay('custom-scroll-position-row',!!c.custom_scroll_position);
setDisplay('custom-scroll-position-separator',!!c.custom_scroll_position);
...
def html():
    ...
    if if_long == 128:
        ...
        try:
            ...
            _scroll_profile = {}
        _scroll_show = "" if bool((_scroll_profile.get('capabilities', {}) or {}).get('scroll_speed', False)) else "display:none;"
        scroll_html = ''.join([
    ...
    # Resolve renderer capabilities once for the active View.
    ...
    _active_caps = _active_profile.get('capabilities', {}) or {}
    _active_defaults = _active_profile.get('defaults', {}) or {}
    _active_ui = _active_profile.get('ui', {}) or {}
    ...
    _cs_show = "" if bool(_active_caps.get('custom_scroll', False)) else "display:none;"
    _cs_pos_show = "" if bool(_active_caps.get('custom_scroll_position', False)) else "display:none;"
    ...
    _scroll_delay_enabled = (_scroll_content == "disruptions" or
                             (bool(_active_caps.get('message_interval', False)) and _scroll_content == "custom"))
    ...
    custom_scroll_html = ''.join([
        ...
        '<div id="dlr-scroll-mode-row" style="', ("" if bool(_active_caps.get('custom_scroll', False)) else "display:none;"), 'margin-bottom:10px">',
        ...
        '<div id="custom-scroll-text-row" style="opacity:', (_scroll_custom_opacity if bool(_active_caps.get('custom_scroll', False)) else "1"), '">',
        ...
        (_scroll_custom_disabled if bool(_active_caps.get('custom_scroll', False)) else ""), '></div>',

        '<div style="border-top:1px solid var(--border);margin:10px 0 0"></div>',
        '<div id="dlr-scroll-delay-row" class="toggle-row" style="border-bottom:none;', ("" if bool(_active_caps.get('message_interval', False)) else "display:none;"), ';opacity:', _scroll_delay_opacity, '">',
        ...
        '<div id="custom-scroll-position-row" class="toggle-row" style="border-bottom:none;', _cs_pos_show, ';opacity:', (_scroll_custom_opacity if bool(_active_caps.get('custom_scroll_position', False)) else "1"), '"><label for="custom_scroll_position" class="toggle-label">Position</label>',
        ...
        (_scroll_custom_disabled if bool(_active_caps.get('custom_scroll_position', False)) else ""), '>',
    ...
    _clock_list_extra_show = "" if bool(_active_caps.get("clock_extra", False)) else "display:none;"
    clock_row_html = (
        ...
        + '</select></td></tr>'
        + '</table></div>'
        + '<div id="clock-list-color-separator" style="border-top:1px solid var(--border);margin:10px 0;' + ("" if bool(_active_caps.get('clock_extra', False)) else "display:none;") + '"></div>'
    ...
    _clock_show = "" if bool(_active_caps.get('clock', False)) else "display:none;"
    ...
    _line_display_default = int(_active_ui.get("line_display_default", _active_defaults.get("list_line_display", 1)))
    ...
    _view_colour_show = "" if bool(_active_caps.get('line_minute_colour', False)) else "display:none;"
