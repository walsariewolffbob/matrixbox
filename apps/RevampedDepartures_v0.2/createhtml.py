...
def huvudsidan(request):
    ...
    if "set_timer" in request.params:
        ...
        functions.colors()
        if int(varinit.settings["listmode"]): functions.request_view_rebuild()
        return (200, {}, "")
    elif "scroll" in request.params:
        varinit.settings["scroll"] = int(request.params["scroll"])
        # The Classic skin notices this setting change and relearns one clean
        # refresh interval on its next tick. Clearing these shared pacing values
        # makes the transition deterministic in both directions.
        ...
        if varinit.display.width > 64 and varinit.display.height <= 32:
            ...
            if mode in (0, 1, 2):
                ...
                if mode == 2:
                    ...
                    # Returning to the SL Classic default starts at 4
                    ...
                    varinit.settings["maxdest"] = 4
                    varinit.settings["clocktime"] = 0
                    # SL Classic always uses actual transit line numbers.
                    # Restore this after DLR, which intentionally uses 1/2/3.
                    varinit.settings["list_line_display"] = 1
        ...
        try:
            ...
            if int(varinit.settings.get("listmode", 0)) == 2:
                functions.request_view_rebuild()
            elif not int(varinit.settings.get("listmode", 0)):
                varinit.shared["force_view_rebuild"] = 1
            else:
                functions.request_view_rebuild()
        except Exception as e:
