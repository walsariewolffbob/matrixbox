...
# Stage 2F: ViewController is the sole renderer authority, including startup.
# Small/tall displays continue to force the List renderer.
    ...
    while wifi.radio.connected == True and not varinit.exit:            # MAINLOOP ###########################
        ...
        if _requested_mode != _active_view_mode or _rebuild_requested:
            ...
            if _view_changed:
                ...
                # Ordinary setting changes rebuild only the active renderer.
                # A value of 2 keeps the existing explicit splash semantics.
                if _splash_rebuild:
        ...
        # All built-in views now tick through the same controller path.
